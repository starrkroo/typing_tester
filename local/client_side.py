import socket
import sys
from PyQt5.QtWidgets import QLineEdit, QLabel, QApplication, QMainWindow, QPushButton, QWidget, QPlainTextEdit
...
class User(QMainWindow):
    ...
    def initUI(self):
        ...
        self.socket__.connect(('localhost', 5002))

        # configuration of screen -- width, height
        screen_config = QtWidgets.QDesktopWidget().screenGeometry(-1)
        ...
        self.log.setGeometry(0, screen_height // 6, screen_width, int(screen_height * 3/4))
    ...
    def show_data(self):
        self.timer_info.setText(
            str(
                len(self.log.toPlainText())
            ))
        self.timer_info.setFont(QFont('Times', 100))
        self.log.setReadOnly(True)

    def begin_timer(self):
        while self.timer_counter != 60:
